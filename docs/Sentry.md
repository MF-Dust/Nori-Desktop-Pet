...
Sentry 事件不上传聊天内容、提示词、记忆、语音转写、MCP 入参/结果、API Key、Cookie、请求正文或用户身份。错误事件只保留异常类型和可定位的堆栈，事务名只允许固定操作名/路由名。Web 回放全局遮罩文字和输入框并阻断媒体。
