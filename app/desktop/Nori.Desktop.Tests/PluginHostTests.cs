...
public sealed class PluginHostTests
{
	...
	{
		string label = PluginWindowHost.BuildLabel("weather-tool", "main-view");
		Assert.Equal("plugin:weather-tool:main-view", label);
	}

	[Fact]
	public void 正确解析标准插件窗口标签()
	{
		bool success = PluginWindowHost.TryParseLabel("plugin:weather-tool:main-view", out string? pluginId, out string? windowId);
		Assert.True(success);
		Assert.Equal("weather-tool", pluginId);
	...
	{
		FakePluginBridgeSource source = new("trusted-plugin-123", "win-panel");
		PluginDescriptorSummary descriptor = new()
		{
			Id = "trusted-plugin-123",
			Name = "Trusted Plugin",
			Version = "1.0.0",
			Capabilities = ["ui.webview"],
		};

		PluginBridge bridge = new("trusted-plugin-123", "win-panel", descriptor);
		...
		Assert.Equal("trusted-plugin-123", doc.RootElement.GetProperty("id").GetString());
	}

	[Fact]
	public async Task WindowGetInfo严格返回构造绑定的身份()
	{
		FakePluginBridgeSource source = new("my-plugin", "sidebar");
		PluginDescriptorSummary descriptor = new()
		{
			Id = "my-plugin",
			Name = "My Plugin",
		...
		PluginBridge bridge = new("my-plugin", "sidebar", descriptor);
		...
		string json = JsonSerializer.Serialize(result.Value);
		using JsonDocument doc = JsonDocument.Parse(json);
		Assert.Equal("my-plugin", doc.RootElement.GetProperty("pluginId").GetString());
		Assert.Equal("sidebar", doc.RootElement.GetProperty("windowId").GetString());
		Assert.Equal("plugin:my-plugin:sidebar", doc.RootElement.GetProperty("label").GetString());
	}
	...
	[Fact]
	public void 描述符摘要类型不包含InstallPath属性()
	{
		// 静态类型检查: PluginDescriptorSummary 不暴露 InstallPath
		var property = typeof(PluginDescriptorSummary).GetProperty("InstallPath");
	...
	{
		FakePluginBridgeSource source = new("plugin-a", "win-1");
		PluginDescriptorSummary descriptor = new()
		{
			Id = "plugin-a",
			Name = "Plugin A",
		...
		PluginBridge bridge = new("plugin-a", "win-1", descriptor);
	...
	public async Task 白名单命令被允许执行(string cmd)
	{
		...
		PluginBridge bridge = new(
			...
			"win-1",
			descriptor,
			capabilityProvider: _ => ["ui.webview", "custom.cap"],
			closeSelfHandler: ct =>
			{
				closeCalled = true;
		...
		if (cmd is "window_close" or "window.close" or "close")
		{
	...
	public async Task 回调抛出异常时被包装为稳定的Reject响应()
	{
		...
		PluginBridge bridge = new(
			...
			"win-1",
			descriptor,
			capabilityProvider: _ => throw new InvalidOperationException("内部能力查询失败，涉及路径 C:\\Users\\Secret\\test"));
	...
	public async Task PluginWebViewCapability校验合法参数并委托工厂创建()
	{
		...
		FakePluginWebViewWindow fakeWindow = new("test-plugin", "win-view", "Title", "http://127.0.0.1:14201/plugins/test/index.html");
		...
		Assert.Equal("plugin:test-plugin:win-view", window.Label);
	}
